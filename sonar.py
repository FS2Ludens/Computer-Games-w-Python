...
def drawBoard(board):
    ...
    #print each row of the 15 rows
    for row in range (15):
        #single digit numbers need to be padded w/ an extra space
        if row < 10:
            extraSpace = ' '
        else:
            extraSpace = ''
...
def getRandomChests(numChests):
    #create a list of chest data structures (two item lists of x, y coordiantes)
    chests = []
...
def makeMove(board, chests, x, y):
    #change the board data structure w/ a sonar device character.
    #remove treasure chests from the chests list as tehy are found
    #return Flase if this is an invalid move
...
the ocean. But you only have cheap sonars that finds distance, not direction.
    ...
    while sonarDevices > 0:
        ...
        if moveResult == False:
            ...
            if moveResult == 'You have found a sunken treasure chest!':
                #upadte all sonar devices currently on the map
        ...
        if len(theChests) == 0:
            print('You have found all sunken treasure chests! Congratulations and good game!')
            break
